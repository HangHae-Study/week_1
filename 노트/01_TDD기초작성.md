...
강결합을 지양하는 테스트 코드를 작성하라고 하셨지만, 기존에 테이블 클래스가 어떤 형태를 가지고 있는지 먼저 확인한 후에 서비스 클래스와 테스트 코드를 작성하였음.
...
1. updateTimeMillies 필드가 존재하는 것을 보았을 때, getOrDefault할 때 하기 전에 현재 currentTimeMillies를 호출해서 기능 호출 시점 값으 하나 저장하고
2. 이미 저장되었던 레코드는 updateTimeMillies < currentTimeMillies 가 될거고, 새로운 레코드는 updateTimeMilleis > currentTimeMillies가 될 것 같음..!